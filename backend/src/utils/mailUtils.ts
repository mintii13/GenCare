...
export class MailUtils{
    ...
    const isAbnormal = (testName: string, value: any): boolean => {
    ...
    const createTestSection = (testData: any, title: string, testCode?: string): string => {
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background-color: #fafafa;">
                                <th style="padding: 12px; text-align: left; border-bottom: 1px solid #e8e8e8; font-weight: bold; width: 35%;">Xét nghiệm</th>
                                <th style="padding: 12px; text-align: left; border-bottom: 1px solid #e8e8e8; font-weight: bold; width: 30%;">Kết quả</th>
                                <th style="padding: 12px; text-align: left; border-bottom: 1px solid #e8e8e8; font-weight: bold; width: 35%;">Giá trị tham chiếu</th>
                            </tr>
                        </thead>
                        <tbody>
        `;
        ...
        Object.keys(testData).forEach(key => {
            if (testData[key] !== undefined && testData[key] !== null) {
                ...
                let rowStyle = '';
                if (abnormal || (typeof value === 'boolean' && value === true)) {
                    rowStyle = 'background-color: #d7d3ccff;'; // Light red background for abnormal/positive results
                }
                
                section += `
                    <tr style="${rowStyle}">
                        <td style="padding: 12px; border-bottom: 1px solid #f0f0f0; font-weight: 500;">${formatTestName(key)}</td>
                        <td style="padding: 12px; border-bottom: 1px solid #f0f0f0; ${status.style} ${abnormal ? 'color: #ffeeba;' : ''}">${formattedValue}</td>
                        <td style="padding: 12px; border-bottom: 1px solid #f0f0f0; color: #666; font-size: 13px;">${normalRange}</td>
                    </tr>
                `;
            }
        });
...
<<<<<<< HEAD
    ...
    const summaryHtml = stiResult.sti_result_items.map((item, idx) => {
        ...
        const testInfo = item.sti_test_id as any;
        const testCode = testInfo?.sti_test_code || '';
        const testName = testInfo?.sti_test_name || 'Xét nghiệm STI';
            ...
            <div style="margin-bottom: 32px; padding: 20px; border: 2px solid #e6f7ff; border-radius: 12px; background-color: #fafffe;">
                <div style="margin-bottom: 16px; padding: 16px; background-color: #e6f7ff; border-radius: 8px;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 16px;">
                        <div><strong>Mã xét nghiệm:</strong> <code style="background-color: #f5f5f5; padding: 2px 6px; border-radius: 4px;">${testCode}</code></div>
        ...
        if (testInfo?.description) {
            itemHtml += `
                <div style="margin-top: 16px; padding: 12px; background-color: #fafafa; border-radius: 6px; border-left: 4px solid #1890ff;">
                    ...
                    <span style="color: #666;">${testInfo.description}</span>
                </div>
            `;
        }
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 20px;">
            ...
            <div style="margin-bottom: 32px; padding: 20px; background-color: #e3f2fd; border-radius: 8px;">
                ...
                <div style="color: #1565c0;">
                    <p style="margin: 8px 0;"><strong>Phòng khám STI</strong></p>
                    <p style="margin: 8px 0;">📧 Email: ${process.env.EMAIL_FOR_VERIFY}</p>
            ...
            <div style="text-align: center; padding: 20px; border-top: 1px solid #e9ecef; color: #6c757d; font-size: 14px;">
                <p style="margin: 0;">Email này được gửi tự động từ hệ thống phòng khám STI</p>
