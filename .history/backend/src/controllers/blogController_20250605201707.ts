...
router.put('/:blogId', authenticateToken, validateCreateBlog, async (req: Request, res: Response) => {
    try {
        ...
        if (result.success) {
            ...
            res.status(404).json(result);
        } else if (result.message === 'Bạn không có quyền chỉnh sửa blog này') {
...
router.put('/:blogId/comments/:commentId', authenticateToken, validateCreateBlogComment, async (req: Request, res: Response) => {
    try {
        ...
        if (result.success) {
            res.status(200).json(result);
        } else if (result.message === 'Không tìm thấy comment') {
            res.status(404).json(result);
        } else if (result.message === 'Bạn không có quyền chỉnh sửa comment này') {
            res.status(403).json(result);
        } else {
            res.status(400).json(result);
        }
