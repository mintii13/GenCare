...
export class AppointmentService {
    ...
    public static async cancelAppointment(
        ...
        try {
            ...
            // Kiểm tra quyền cancel
            const canCancel = this.canUserModifyAppointment(appointment, cancelledBy, requestUserRole);
            if (!canCancel) {
                return {
                    success: false,
                    message: 'You do not have permission to cancel this appointment'
            ...
            const cancelledAppointment = await AppointmentRepository.cancelById(
                appointmentId,
                cancelledBy,
                reason
            );
    ...
    public static async rescheduleAppointment(
        ...
        try {
            ...
            const newDuration = parseTime(newEndTime) - parseTime(newStartTime);
