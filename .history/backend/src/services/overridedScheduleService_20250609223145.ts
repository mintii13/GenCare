...
export class OverridedScheduleService {
    ...
    public static async getDaySchedule(
        ...
        try {
            ...
            if (override) {
                // Nếu có override unavailable, trả về empty slots
                ...
                // Nếu có override custom_hours, tạo slots từ override
                if (override.override_type === 'custom_hours' && override.custom_start_time && override.custom_end_time) {
                    const slots = this.generateTimeSlots(
                        override.custom_start_time,
                        override.custom_end_time,
                        override.custom_breaks || []
                    );

                    return {
                        success: true,
                        message: 'Schedule retrieved successfully (custom hours)',
                        data: {
                            date: date.toISOString().split('T')[0],
                            consultant_id: consultantId,
    ...
    /**
     * Tạo danh sách các time slot từ thời gian làm việc - FIXED VERSION
     */
    private static generateTimeSlots(
        ...
        const breakPeriods = breaks.map(breakTime => ({
            start: parseTime(breakTime.start_time),
            end: parseTime(breakTime.end_time)
        }));
        ...
        while (currentTime + slotDuration <= endMinutes) {
            ...
            // Check if slot conflicts with any break period
            const conflictsWithBreak = breakPeriods.some(breakPeriod => {
                // Slot conflicts if it overlaps with break period
                return (currentTime < breakPeriod.end && slotEndTime > breakPeriod.start);
            });
            ...
            const currentBreak = breakPeriods.find(breakPeriod =>
                currentTime >= breakPeriod.start && currentTime < breakPeriod.end
            );

            if (currentBreak) {
                currentTime = currentBreak.end;
            }
        }
