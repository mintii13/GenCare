...
export class ConsultantScheduleService {
    ...
    /**
     * Lấy danh sách lịch làm việc của consultant hoặc tất cả schedules (cho staff/admin)
     */
    ...
    public static async updateSchedule(
        ...
        try {
            ...
            if (userRole === 'admin') {
                // Admin có thể update tất cả schedules
                canUpdate = true;
            } else if (userRole === 'staff') {
                // Staff có thể update tất cả schedules
                canUpdate = true;
            } else if (userRole === 'consultant') {
                // Consultant chỉ có thể update schedule của chính mình
                const consultant = await Consultant.findOne({ user_id: userId });
                if (consultant && existingSchedule.consultant_id.toString() === consultant._id.toString()) {
                    canUpdate = true;
                }
            }
    ...
    public static async deleteSchedule(
        ...
        try {
            ...
            if (userRole === 'admin') {
                // Admin có thể delete tất cả schedules
                canDelete = true;
            } else if (userRole === 'staff') {
                // Staff có thể delete tất cả schedules
                canDelete = true;
            } else if (userRole === 'consultant') {
                // Consultant chỉ có thể delete schedule của chính mình
                const consultant = await Consultant.findOne({ user_id: userId });
                if (consultant && existingSchedule.consultant_id.toString() === consultant._id.toString()) {
                    canDelete = true;
                }
