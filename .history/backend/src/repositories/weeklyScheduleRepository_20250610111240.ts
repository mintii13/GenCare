...
export class WeeklyScheduleRepository {
     ...
     */
    public static async findAll(): Promise<IWeeklySchedule[]> {
        try {
    ...
    public static async updateByConsultantId(
        ...
        try {
            // Log để debug
            console.log('Updating schedule for consultant:', consultantId);
            console.log('Update data:', JSON.stringify(updateData, null, 2));

            const result = await WeeklySchedule.findOneAndUpdate(
                { consultant_id: consultantId },
                {
                    ...updateData,
