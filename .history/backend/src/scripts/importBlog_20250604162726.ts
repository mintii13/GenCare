...
async function checkDatabase() {
    try {
        ...
        const [users, consultants, customers, blogs, comments] = await Promise.all([
            User.find().lean(),
            Consultant.find().lean(),
            Customer.find().lean(),
            Blog.find().lean(),
            BlogComment.find().lean()
        ]);

        console.log('\n📊 DATABASE STATUS:');
        console.log(`👥 Users: ${users.length}`);
        users.forEach(user => console.log(`   - ${user.full_name} (${user.email})`));

        console.log(`🩺 Consultants: ${consultants.length}`);
        consultants.forEach(consultant => console.log(`   - User ID: ${consultant.user_id}`));

        console.log(`👩‍⚕️ Customers: ${customers.length}`);
        customers.forEach(customer => console.log(`   - User ID: ${customer.user_id}`));

        console.log(`📝 Blogs: ${blogs.length}`);
        blogs.forEach(blog => console.log(`   - ${blog.title.substring(0, 50)}... (Author: ${blog.author_id})`));

        console.log(`💬 Comments: ${comments.length}`);
        comments.forEach(comment => console.log(`   - Blog: ${comment.blog_id}, Customer: ${comment.customer_id || 'Anonymous'}`));
