...
const createOverridedScheduleSchema = Joi.object({
    ...
    if (value.override_type === 'custom_hours') {
        ...
        if (value.custom_break_start && value.custom_break_end) {
            const breakStartTime = value.custom_break_start.split(':').map(Number);
            const breakEndTime = value.custom_break_end.split(':').map(Number);
            ...
            if (breakStartMinutes >= breakEndMinutes) {
                return helpers.error('any.invalid', {
                    message: 'Custom break start time must be before custom break end time'
                });
            }

            if (breakStartMinutes < startMinutes || breakEndMinutes > endMinutes) {
                return helpers.error('any.invalid', {
                    message: 'Custom break time must be within custom working hours'
                });
            }
        } else if ((value.custom_break_start && !value.custom_break_end) || (!value.custom_break_start && value.custom_break_end)) {
            return helpers.error('any.invalid', {
                message: 'Both custom break start and end time are required if custom break is specified'
            });
        }
    }
